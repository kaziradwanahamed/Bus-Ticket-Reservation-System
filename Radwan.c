...
void viewSeats() {
    // Open a file to save seat details
    ...
    for (int i = 1; i <= totalSeats; i++) {
        if (!seats[i - 1].isBooked) {
            ...
            fprintf(file, "Seat %d is available\n", i); // Save to file
        } else {
            printf("Seat %d is booked by %s\n", i, seats[i - 1].bookedBy);
            fprintf(file, "Seat %d is booked by %s\n", i, seats[i - 1].bookedBy); // Save to file
        }
    }

    fclose(file); // Close the file
    printf("Seat details have been saved to 'seats.txt'.\n");
}
...
void bookTicket() {
    ...
    if (seats[seatNumber - 1].isBooked) {
        ...
        // Update the file with the new seat data
        FILE *file = fopen("seats.txt", "w");
        if (file == NULL) {
            printf("Error: Unable to open file.\n");
            return;
        ...
        fclose(file);
        printf("Seat details have been updated in 'seats.txt'.\n");
    }
}
